...
data_path="C:/Users/Afrid Shirsekar/Downloads/MLDS_hw2_2_data/MLDS_hw2_1_data/"
model_path='Model'
...
def string_split(sentence, dictonary, w2i):  
...
def evaluate(test_loader, model):
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        val1, val2 = val1.to(device), val2.to(device)
...
def testfun(test_loader, model, i2w):
    ...
    for batch_idx, batch in enumerate(test_loader):
        id, f1 = batch
        id, f1 = id, Variable(f1).float().to(device)
...
def main():
    ...
    modeltrain = model.ModelMain(encoder=encode, decoder=decode).to(device)
    ...
    torch.save(modeltrain, "{}/{}.h5".format(ModelSaveLoc, 'modelarch'))
